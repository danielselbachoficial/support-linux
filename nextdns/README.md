# NextDNS no Linux: Guia de Instalação e Configuração

Repositório dedicado a documentar a instalação, otimização e resolução de problemas do NextDNS em sistemas Linux (Ubuntu, Debian, Fedora, etc.).

## 📌 Conteúdo
1. [O que é o NextDNS?](./O-QUE-E-NEXTDNS.md) - Entenda a ferramenta e os planos (Free vs Pro).
2. [Guia de Instalação](./INSTALACAO.md) - Passo a passo para CLI, Systemd-Resolved e outros.
3. [Solução de Problemas](./INSTALACAO.md#resolvendo-erro-de-hostname) - Como corrigir o erro de `sudo` e hostname no Ubuntu.

## 🚀 Início Rápido (Ubuntu/Debian)
Para instalar o cliente oficial via linha de comando:

```bash
sh -c "$(curl -sL [https://nextdns.io/install](https://nextdns.io/install))"
